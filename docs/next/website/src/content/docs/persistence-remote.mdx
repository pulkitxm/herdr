...
Direct attach streams the current rendered terminal state, then live ANSI frames. Input goes straight to that terminal. Kitty images are included when graphics are enabled and the outer terminal reports its cell dimensions.
...
Controllers must send their pixel cell dimensions in an initial resize command to receive Kitty images without waiting for a window resize:

```json
{"type":"terminal.resize","cols":120,"rows":40,"cell_width_px":8,"cell_height_px":16}
```

Image uploads and placements are embedded in the ANSI bytes of `terminal.frame` records. Preserve those bytes when forwarding frames to a Kitty graphics compatible terminal.
